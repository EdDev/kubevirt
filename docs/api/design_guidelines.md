...
near-zero CEL adoption, and custom condition structs without per-condition
`observedGeneration`. Where the document calls out known gaps explicitly, those
...
- **Hypervisor:** Avoid embedding `qemu`- or `libvirt`-specific concepts. Use
  ...
  The temptation is highest at the virtualization boundary, which is why the
...
- **Patterns:** Use owner references for garbage collection and standard
  ...
  #390](https://github.com/kubevirt/enhancements/pull/390). The cost of
...
hardware identity, and the hypervisor abstraction layer are canonical examples.
...
user manages, rather than into `status`.
...
- **Fractional or Decimal Weights:** Raw floating-point numbers are strictly
  ...
  > `spec` is written by multiple actors across different languages and client
...
In accordance with upstream [Kubernetes
...
should be treated as a secondary fallback reserved exclusively for highly
dynamic, external database lookups.

> **Note:** KubeVirt's existing validation logic lives predominantly in
> imperative admission webhooks, which predate this requirement. These must not
...
- **Unsafe CEL Usage (Retroactive Restrictions):** Developers must not introduce
  ...
  it, this mode is the safe way to tighten validation on an existing field.
...
during the active execution loop. The control plane must calculate the new
status entirely from the current observed state of the world, rather than
computing it as a delta, increment, or state transition from the *previous*
reconciliation cycle's status data.
...
The table below is a representative reference, not exhaustive.
